...
//------------------------------------------------------------------------
void CGridColumnTraitEdit::OnCustomDraw(CGridListCtrlEx& owner, NMLVCUSTOMDRAW* pLVCD, LRESULT* pResult)
{
	int nRow = (int)pLVCD->nmcd.dwItemSpec;

	const bool bIsPassword = (m_EditStyle & ES_PASSWORD) ? true : false;
	...
	switch (pLVCD->nmcd.dwDrawStage)
	{
		...
		case CDDS_ITEMPOSTPAINT | CDDS_SUBITEM:
		{
			...
			CString csTxt(owner.GetItemText(nRow,nCol));
			
			CString csPass(_T(""));
			for(int i=0; i<csTxt.GetLength(); i++)
				csPass.Insert(0, _T("*"));
			...
			{	
				rcEdit.left += rcIcon.Width();					// Shrink rcCell from rcIcon
				rcEdit.left = min(rcEdit.left,rcEdit.right);	// priority to Icon
			}
			...
			pDC->FillSolidRect(rcEdit, GetSysColor(COLOR_BTNHIGHLIGHT));//COLOR_BTNFACE));

			HDITEM hditem = {0};
			hditem.mask = HDI_FORMAT;
			VERIFY( owner.GetHeaderCtrl()->GetItem(nCol, &hditem) );
			if (hditem.fmt & HDF_CENTER)
				pDC->DrawText(csPass, &rcEdit, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
			else if (hditem.fmt & HDF_RIGHT)
				pDC->DrawText(csPass, &rcEdit, DT_RIGHT|DT_VCENTER|DT_SINGLELINE);
			else
				pDC->DrawText(csPass, &rcEdit, DT_LEFT|DT_VCENTER|DT_SINGLELINE);
		} break;
	}
