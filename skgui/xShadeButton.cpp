...
void CxShadeButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	int yMargin = (m_TextAlign&DT_SINGLELINE) ? 0 : 4;
	RECT tr={r.left+m_FocusRectMargin+2,r.top+yMargin,r.right-m_FocusRectMargin-2,r.bottom-yMargin};
...
//	pDC->GetTextMetrics(&tm);							// get font metrics
	...
	if (lpDrawItemStruct->itemState & ODS_DISABLED){	// DISABLED BUTTON
		...
		if(m_Checked){
			if (m_IconDown)	//draw the icon
				pDC->DrawState(m_IconBox.TopLeft(),m_IconBox.Size(),
					m_IconDown,DSS_DISABLED,(CBrush*)NULL);
		}
		else{
...
/////////////////////////////////////////////////////////////////////////////
void CxShadeButton::SetIconAuto(LPWSTR strPath, LPWSTR strIcon, UINT nIconAlign, LPWSTR strIconDown, LPWSTR strIconHighLight)
{
	if (m_IconDown!=m_Icon && m_IconDown) DestroyIcon(m_IconDown);
	if (m_IconHighLight!=m_Icon && m_IconHighLight) DestroyIcon(m_IconHighLight);
	if (m_Icon) DestroyIcon(m_Icon);
	...
	WCHAR strIconPath[MAX_PATH];
	swprintf_s(strIconPath, L"%s\\%s", strPath, strIcon);
	...
	if (m_Icon){							// if success...
		...
		if (strIconDown){	//load down icon
			swprintf_s(strIconPath, L"%s\\%s", strPath, strIconDown);
		...
		if (strIconHighLight){	//load highlighted icon
			swprintf_s(strIconPath, L"%s\\%s", strPath, strIconHighLight);
...
void CxShadeButton::SetIcon(LPWSTR strIcon, UINT nIconAlign, LPWSTR strIconDown, LPWSTR strIconHighLight, int cx, int cy)
{
	if (m_IconDown!=m_Icon && m_IconDown) DestroyIcon(m_IconDown);
	if (m_IconHighLight!=m_Icon && m_IconHighLight) DestroyIcon(m_IconHighLight);
	if (m_Icon) DestroyIcon(m_Icon);
...
void CxShadeButton::SetIcon(UINT nIcon, UINT nIconAlign, UINT nIconDown, UINT nIconHighLight, int cx, int cy)
{
	if (m_IconDown!=m_Icon && m_IconDown) DestroyIcon(m_IconDown);
	if (m_IconHighLight!=m_Icon && m_IconHighLight) DestroyIcon(m_IconHighLight);
	if (m_Icon) DestroyIcon(m_Icon);
...
//	see DrawText() styles...
	...
	m_TextAlign |= DT_VCENTER;
	CString sCaption;
	GetWindowText(sCaption);
	if(sCaption.Find('\n', 0) < 0)	m_TextAlign |= DT_SINGLELINE;
}
/////////////////////////////////////////////////////////////////////////////
